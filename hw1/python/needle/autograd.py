    ...
    for node in reverse_topo_order:
        ...
        if node.op is not None:
            ...
            for input_node, grad in zip(node.inputs, input_grads):
                # 注意区分grad和tensor，tensor就是forward时建立的tensor，grad是backward时建立的tensor
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    """
    visited = set()
    # 也可以给node打颜色，这样就不需要visited了
    topo_order = []
    # 注意这里是从output_node开始的，因此需要reverse
    for node in node_list:
        # 其实只有output_node会被访问到
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    # node之后的节点已经遍历完毕，可以加入到topo_order中
